...
include("include/Ticket_Demo_Site_variables.php");
...
$layout->blocks["top"][] = "export";$page_layouts["Ticket_Demo_Site_export"] = $layout;
} else {
$layout = new TLayout("export","FancyPink","MobilePink");
$layout->blocks["top"] = array();
$layout->containers["export"] = array();
...
$xt->display("Ticket_Demo_Site_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=Ticket_Demo_Site.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Ticket_Demo_Site.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Ticket_Demo_Site.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["location"] = DisplayLookupWizard("location",$row["location"],$row,"",MODE_EXPORT, PAGE_EXPORT);		
			$values["VRM"] = GetData($row, "VRM", "", PAGE_EXPORT);
			$values["make"] = DisplayLookupWizard("make",$row["make"],$row,"",MODE_EXPORT, PAGE_EXPORT);		
			$values["colour"] = DisplayLookupWizard("colour",$row["colour"],$row,"",MODE_EXPORT, PAGE_EXPORT);		
			$values["notes"] = GetData($row, "notes", "", PAGE_EXPORT);
			$values["date"] = GetData($row, "date", "", PAGE_EXPORT);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Ticket_Demo_Site.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["colour"] = DisplayLookupWizard("colour", $row["colour"], $row, "", MODE_EXPORT, PAGE_EXPORT);
				$format = "";
			$values["notes"] = GetData($row, "notes", $format, PAGE_EXPORT);
				$format = "Short Date";
			$values["date"] = GetData($row, "date", $format, PAGE_EXPORT);
...
// write header
	...
	echo "</tr>";
		...
		$totalsFields[] = array('fName'=>"colour", 'totalsType'=>'', 'viewFormat'=>"");
			$totals["notes"] = array("value" => 0, "numRows" => 0);
		$totalsFields[] = array('fName'=>"notes", 'totalsType'=>'', 'viewFormat'=>"");
			$totals["date"] = array("value" => 0, "numRows" => 0);
		$totalsFields[] = array('fName'=>"date", 'totalsType'=>'', 'viewFormat'=>"Short Date");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			...
			{
				$values["location"] = DisplayLookupWizard("location", $row["location"], $row, ""
					, MODE_EXPORT, PAGE_EXPORT);
			}
									$format = "";
			...
			if(strlen($row["make"]))
			{
				$values["make"] = DisplayLookupWizard("make", $row["make"], $row, ""
					, MODE_EXPORT, PAGE_EXPORT);
			}
					$values["colour"] = "";	
			if(strlen($row["colour"]))
			{
				$values["colour"] = DisplayLookupWizard("colour", $row["colour"], $row, ""
					, MODE_EXPORT, PAGE_EXPORT);
			}
									$format = "";
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
						...
						echo htmlspecialchars($values["notes"]);
			echo '</td>';
							echo '<td>';
