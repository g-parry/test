...
$layout->blocks["top"][] = "details";$page_layouts["isha_audit_edit"] = $layout;
} else {
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "isha_audit_inline_edit.htm" : "isha_audit_edit.htm";
...
//	processing datetime - begin
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "datetime"=="password" && $url_page=="admin_users_")
				$value = md5($value);
...
//	processing ip - begin
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "ip"=="password" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["ip"] = $value;
...
//	processing user - begin
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "user"=="password" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["user"] = $value;
...
//	processing table - begin
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "table"=="password" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["table"] = $value;
...
//	processing description - begin
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "description"=="password" && $url_page=="admin_users_")
				$value = md5($value);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: isha_audit_".$pageObject->getPageType().".php?".$keyGetQ);
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("ip_label","<label for=\"".GetInputElementId("ip", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("user_label","<label for=\"".GetInputElementId("user", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("table_label","<label for=\"".GetInputElementId("table", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("action_label","<label for=\"".GetInputElementId("action", $id, PAGE_EDIT)."\">","</label>");
...
//	datetime - Short Date
		$value = "";
			$value = ProcessLargeText($pageObject->pSet, GetData($data, "datetime", "Short Date", PAGE_LIST), "field=datetime".$keylink, "", MODE_LIST);
...
//	ip - 
		$value = "";
			$value = ProcessLargeText($pageObject->pSet, GetData($data, "ip", "", PAGE_LIST), "field=ip".$keylink, "", MODE_LIST);
	$showValues["ip"] = $value;
	$showFields[] = "ip";
		$showRawValues["ip"] = substr($data["ip"],0,100);

//	user - 
		$value = "";
			$value = ProcessLargeText($pageObject->pSet, GetData($data, "user", "", PAGE_LIST), "field=user".$keylink, "", MODE_LIST);
	$showValues["user"] = $value;
	$showFields[] = "user";
		$showRawValues["user"] = substr($data["user"],0,100);

//	table - 
		$value = "";
			$value = ProcessLargeText($pageObject->pSet, GetData($data, "table", "", PAGE_LIST), "field=table".$keylink, "", MODE_LIST);
	$showValues["table"] = $value;
	$showFields[] = "table";
		$showRawValues["table"] = substr($data["table"],0,100);

//	action - 
		$value = "";
			$value = ProcessLargeText($pageObject->pSet, GetData($data, "action", "", PAGE_LIST), "field=action".$keylink, "", MODE_LIST);
	$showValues["action"] = $value;
	$showFields[] = "action";
		$showRawValues["action"] = substr($data["action"],0,100);

//	description - 
		$value = "";
			$value = ProcessLargeText($pageObject->pSet, GetData($data, "description", "", PAGE_LIST), "field=description".$keylink, "", MODE_LIST);
...
if($pageObject->isHTMLFormNeeded())
{
	...
	$pageObject->body["begin"] .= '<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="isha_audit_edit.php">';
}
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='isha_audit_view.php?".$viewlink."'\"");
