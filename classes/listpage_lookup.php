...
class ListPage_Lookup extends ListPage_Embed
{
	...
	function initLookupParams()
	{
		...
		$lookupPSet = new ProjectSettings($this->mainTable, PAGE_SEARCH);
...
// clear lookup session data, while loading at first time
	...
	function proccessRecordValue(&$data, &$keylink, $listFieldInfo)
	{
		...
		{
			$value = ProcessLargeText($this->pSet, GetData($data, $this->linkField, $listFieldInfo['viewFormat'], PAGE_LIST), "field=".rawurlencode($listFieldInfo['fName']).$keylink, "", MODE_LIST);				
		}
		else
		{
	...
	function showPage() 
	{
		...
		if (isMobile()){
			...
			$bricksExcept = array("grid_mobile","pagination","vsearch2","cancelbutton_mobile");
		}
		else {
