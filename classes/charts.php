...
class Chart
{
	...
	function Chart(&$ch_array, $param)
	{
		...
		{	
			...
			if(isset($_SESSION[$this->sessionPrefix.'_advsearch']))
			{
				...
				$strWhereClause = $searchClauseObj->getWhere($this->pSet->getListOfFieldsByExprType(false));
				$searchHavingClause = $searchClauseObj->getWhere($this->pSet->getListOfFieldsByExprType(true));
	...
	function labelFormat($fieldName,$data)
	{
		...
		if(($strEditFormat == EDIT_FORMAT_LOOKUP_WIZARD || $strEditFormat == EDIT_FORMAT_RADIO) 
			&& ($this->pSet->getLookupType($fieldName) == LT_LOOKUPTABLE 
				|| $this->pSet->getLookupType($fieldName) == LT_QUERY) 
			&& $this->pSet->getLWLinkField($fieldName) != $this->pSet->getLWDisplayField($fieldName)) 
		{
			$value = DisplayLookupWizard($fieldName, $data[$fieldName], $data, "", MODE_PRINT, PAGE_CHART);
		} 
		else
		{				
			$value = GetData($data, $fieldName, $strViewFormat, PAGE_CHART);
		} 
		if(strlen($value)>50)
		{
