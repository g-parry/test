...
class EditPage extends RunnerPage
{
	...
	function assignBodyEnd(&$params) 
	{
		...
		{
		 	$index = 1;
		 	foreach ($this->jsSettings["tableSettings"][$this->tName]['keys'] as $key)
		 	{
		 		echo '<input type="hidden" name="editid'.$index.'" value="'.$key.'" />';
		 		$index++;
		 	}
		 	echo '<input type="hidden" name="a" value="edited" /><input type="hidden" name="editType" value="'.EDIT_SIMPLE.'" /><input type="hidden" name="id" value="'.$this->id.'" /></form>';
		}
	}
