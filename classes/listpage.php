...
{	
	...
	{	
		...
		foreach($this->selectedRecs as $keys) 
		{
			...
			if($this->lockingObj)
			{
				...
				$lockSQL = "select * from ".AddTableWrappers("dispensations_locking")." where ".AddFieldWrappers("keys")."=".db_prepare_string($lockWhere)." and ".AddFieldWrappers("table")."=".db_prepare_string($this->origTName)." and ".AddFieldWrappers("action")."=1";
			...
			if(!$lockRecord && @$_REQUEST["a"] == "delete" && $retval) 
			{
				...
				if($this->delFile)
					DeleteUploadedFiles($this->pSet, $this->gQuery, $where, PAGE_LIST);
	...
	function buildSQL() 
	{
		...
		$searchWhereClause = $this->searchClauseObj->getWhere($this->pSet->getListOfFieldsByExprType(false));
	...
	function proccessRecordValue(&$data, &$keylink, $listFieldInfo)
	{
		...
		{
			if($this->pSet->showThumbnail($listFieldInfo['fName'])) 
			{
				
				$thumbPref = $this->pSet->getStrThumbnail($listFieldInfo['fName']);
				$value.= "<a";
				if($this->pSet->isUseiBox($listFieldInfo['fName']))
					$value.= " rel='ibox'";
				else
					$value.= " target=_blank";
				
				$value.= " href='imager.php?table=".$this->shortTableName."&field=".rawurlencode($listFieldInfo['fName']).$keylink."&rndVal=".rand(0,32768)."'>";
				$value.= "<img border=0";
				if($this->is508)
					$value.= " alt=\"Image from DB\"";
				$value.= " src='imager.php?table=".$this->shortTableName."&field=".rawurlencode($thumbPref)."&alt=".rawurlencode($listFieldInfo['fName']).$keylink."&rndVal=".rand(0,32768)."'>";
				$value.= "</a>";
			} 
			else 
			{
				...
				$imgWidth = $this->pSet->getImageWidth($listFieldInfo['fName']);
				$value.=($imgWidth ? " width=".$imgWidth : "");
				
				$imgHeight = $this->pSet->getImageHeight($listFieldInfo['fName']);
				$value.=($imgHeight ? " height=".$imgHeight : "");
				
				$value.= " border=0";
				$value.= " src='imager.php?table=".$this->shortTableName."&field=".rawurlencode($listFieldInfo['fName']).$keylink."&rndVal=".rand(0,32768)."'>";
			}
		}
		else if($listFieldInfo['viewFormat'] == FORMAT_FILE_IMAGE) 
		{
			if(CheckImageExtension($data[$listFieldInfo['fName']])) 
			{
				if($this->pSet->showThumbnail($listFieldInfo['fName'])) 
				{
					...
					if(substr($this->pSet->getLinkPrefix($listFieldInfo['fName']), 0, 7) != "http://" && ! myfile_exists($this->pSet->getUploadFolder($listFieldInfo['fName']).$thumbname))
					...
					$value.= " href=\"".htmlspecialchars(AddLinkPrefix($this->pSet, $listFieldInfo['fName'], $data[$listFieldInfo['fName']]))."\">";
					$value.= "<img";
					...
					$value.= " src=\"".htmlspecialchars(AddLinkPrefix($this->pSet, $listFieldInfo['fName'], $thumbname))."\"></a>";
				} 
				else 
				{
					$value = "<img";
					...
					if($this->is508)
						$value.= " alt=\"".htmlspecialchars($data[$listFieldInfo['fName']])."\"";
					$value.= " src=\"".htmlspecialchars(AddLinkPrefix($this->pSet, $listFieldInfo['fName'], $data[$listFieldInfo['fName']]))."\">";
				}
			}
		} 
		else if($listFieldInfo['viewFormat'] == FORMAT_DATABASE_FILE) 
		{
			...
			$fileNameF = $this->pSet->getFilenameField($listFieldInfo['fName']);
			if($fileNameF) {
			...
			$fileName = GetData($data, $listFieldInfo['fName'], FORMAT_NONE, PAGE_LIST, "", $this->pSet, $this->recordFieldTypes[$listFieldInfo['fName']]);
			$fieldIsUrl = $this->pSet->isVideoUrlField($listFieldInfo['fName']);
			if (strlen($fileName)){
				$absFileName = "";
				if(!$fieldIsUrl && $this->pSet->isAbsolute($listFieldInfo['fName']))
					$absFileName = $this->pSet->getUploadFolder($listFieldInfo['fName']).$fileName;
				elseif (!$fieldIsUrl)
					$absFileName = getabspath($this->pSet->getUploadFolder($listFieldInfo['fName']).$fileName);
				
				// if file
				if ($fieldIsUrl || file_exists($absFileName))
				{
					$titleField = $this->pSet->getAudioTitleField($listFieldInfo['fName']);
					$title = "";
					if ($titleField){
						$title = htmlspecialchars(GetData($data, $titleField, $this->pSet->getViewFormat($titleField), PAGE_LIST, "", $this->pSet, $this->recordFieldTypes[$listFieldInfo['fName']]));
					}
					if ($fieldIsUrl){
						$href = $fileName;
					}else{
			...
			if (strlen($fileName)){
				...
				if ($fieldIsUrl || file_exists($absFileName))
				{
					...
					if (isMobileIOS()){
						$value = '<video width="'.$this->pSet->getVideoWidth($listFieldInfo['fName']).'" height="'.$this->pSet->getVideoHeight($listFieldInfo['fName']).'" controls="controls">
								  <source src="'.$href.'"  />
								  Your browser does not support the video tag.
								</video>';
					}
					else {
						$value = '<a href="'.$href.'" style="display:block;width:'
							.$this->pSet->getVideoWidth($listFieldInfo['fName']).'px;height:'
							.$this->pSet->getVideoHeight($listFieldInfo['fName']).'px;" id="'.$videoId.'"></a>';
					}
			...
			if (@$data[$listFieldInfo['fName']] != NULL){
				...
				if (isMobileIOS()){
					$value = '<video width="'.$this->pSet->getVideoWidth($listFieldInfo['fName']).'" height="'.$this->pSet->getVideoHeight($listFieldInfo['fName']).'" controls="controls">
							  <source src="getfile.php?table='.$this->shortTableName.'&field='.rawurlencode($listFieldInfo['fName'])
						.$keylink.'"  />
							  Your browser does not support the video tag.
							</video>';
				}
				else {
					$value = '<a href="getfile.php?table='.$this->shortTableName.'&field='.rawurlencode($listFieldInfo['fName'])
						.$keylink.'" style="display:block;width:'.$this->pSet->getVideoWidth($listFieldInfo['fName']).'px;height:'
						.$this->pSet->getVideoHeight($listFieldInfo['fName']).'px;" id="'.$videoId.'"></a>';
				}
				$this->controlsMap['video'][] = $videoId;
			}
		...
		else if(($listFieldInfo['editFormat'] == EDIT_FORMAT_LOOKUP_WIZARD 
				|| $listFieldInfo['editFormat'] == EDIT_FORMAT_RADIO) 
			&& ($this->pSet->getLookupType($listFieldInfo['fName']) == LT_LOOKUPTABLE
				|| $this->pSet->getLookupType($listFieldInfo['fName']) == LT_QUERY) 
			&& $this->pSet->getLWLinkField($listFieldInfo['fName']) != $this->pSet->getLWDisplayField($listFieldInfo['fName'])) 
		{
			$value = DisplayLookupWizard($listFieldInfo['fName'], $data[$listFieldInfo['fName']], $data, $keylink, MODE_LIST, PAGE_LIST);
		} 
		else if($this->pSet->NeedEncode($listFieldInfo['fName'])) 
		{
			$value = ProcessLargeText($this->pSet, GetData($data, $listFieldInfo['fName'], $listFieldInfo['viewFormat'], PAGE_LIST, "", $this->pSet, $this->recordFieldTypes[$listFieldInfo['fName']]), 
				"field=".rawurlencode($listFieldInfo['fName']).$keylink, "", MODE_LIST, "", $this->mode == LIST_LOOKUP && isMobile());
		}
		else
		{
			$value = GetData($data, $listFieldInfo['fName'], $listFieldInfo['viewFormat'], PAGE_LIST, "", $this->pSet, $this->recordFieldTypes[$listFieldInfo['fName']]);
		}
