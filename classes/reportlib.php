...
class ReportNumericField extends ReportField
{
	...
	{
		$fname = $this->_oldAlgorithm ? GetFullFieldNameForInsert($this->pSet, $this->_name) : cached_ffn($this->_name);
		if($this->_interval > 0)
		{
...
class ReportDateField extends ReportField
{
	...
	function getFieldName($fieldValue, $data)
	{
		...
		if($this->_interval == 0)
		{
			if($this->_viewFormat)
			{
				return GetDataInt($value, array($this->_name=>$value), $this->_name, $this->_viewFormat, PAGE_REPORT);
			}
			else
			{
				$date = db2time($value);
...
function getFormattedValue($value, $fieldName, $strViewFormat, $strEditFormat = '', $mode = MODE_LIST)
{
	...
	else
	{
		if($strEditFormat == EDIT_FORMAT_LOOKUP_WIZARD || $strEditFormat == EDIT_FORMAT_RADIO)
			$val = DisplayLookupWizard($fieldName, $value, null, '', $mode, PAGE_REPORT);
		else
			$val = GetDataInt($value, array($fieldName=>$value), $fieldName, $strViewFormat, PAGE_REPORT);
	}
...
//		return $this->buildsql($hsql);
	...
	{
		global $gQuery;
		return $this->searchClauseObj->applyWhere($sql, $this->pSet->getListOfFieldsByExprType(false), $this->pSet->getListOfFieldsByExprType(true));
	}
		...
		{		
			...
			if ($this->fieldsArr[$i]['viewFormat'] == FORMAT_DATABASE_IMAGE)
			{
				if(!$this->forExport)
				{
					...
					{
						$val.="<a ";
						if($this->pSet->useIbox($this->fieldsArr[$i]['name']))
							$val.= " rel='ibox'";
						else
							$val.= " target=_blank";
				...
				if(!$this->forExport)
				{
					if(CheckImageExtension($value[$this->fieldsArr[$i]['name']])) 
					{
						if ($this->fieldsArr[$i]['showThumb'])
						{
							...
							$val .= " href=\"".htmlspecialchars(AddLinkPrefix($this->pSet, $this->fieldsArr[$i]['name'], $value[$this->fieldsArr[$i]['name']]))."\">";
							$val .= "<img";
							...
							$val .= " src=\"".htmlspecialchars(AddLinkPrefix($this->pSet, $this->fieldsArr[$i]['name'], $thumbname))."\"></a>";
						}else{
							$val = "<img";
							if ($this->fieldsArr[$i]['imageWidth'])
							{
							...
							if(isEnableSection508())
								$val.= " alt=\"".htmlspecialchars($value[$this->fieldsArr[$i]['name']])."\"";
							$val .= " src=\"".htmlspecialchars(AddLinkPrefix($this->pSet, $this->fieldsArr[$i]['name'],$value[$this->fieldsArr[$i]['name']]))."\">";
						}
					}
				}
				else
				...
				if(!$this->forExport)
				{
					...
					{
						$val = "<a href=\"getfile.php?table=".GetTableURL($strTableName)."&filename=".rawurlencode($filename)."&field="
							.rawurlencode(htmlspecialchars($this->fieldsArr[$i]['name'])).$keylink."\">";
			...
			}elseif(($this->fieldsArr[$i]['editFormat'] == EDIT_FORMAT_LOOKUP_WIZARD 
					|| $this->fieldsArr[$i]['editFormat'] == EDIT_FORMAT_RADIO) 
				&& ($this->pSet->getLookupType($this->fieldsArr[$i]['name']) == LT_LOOKUPTABLE 
					|| $this->pSet->getLookupType($this->fieldsArr[$i]['name']) == LT_QUERY)){			
				$val = DisplayLookupWizard($this->fieldsArr[$i]['name'], $value[$this->fieldsArr[$i]['name']], $value, $keylink, $this->mode, PAGE_REPORT);
			}elseif($this->pSet->NeedEncode($this->fieldsArr[$i]['name'])){
				$val = ProcessLargeText($this->pSet, GetData($value, $this->fieldsArr[$i]['name'], $this->fieldsArr[$i]['viewFormat'], PAGE_REPORT), "field=".rawurlencode($this->fieldsArr[$i]['name']).$keylink, "", $this->mode);
			}else{
				if ($this->fieldsArr[$i]['viewFormat'] == FORMAT_CHECKBOX && $this->forExport){
					$val = GetData($value, $this->fieldsArr[$i]['name'], FORMAT_NONE, PAGE_REPORT);
				}else{
					$val = GetData($value, $this->fieldsArr[$i]['name'], $this->fieldsArr[$i]['viewFormat'], PAGE_REPORT);
				}
			}
