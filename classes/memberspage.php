...
{	
	...
	*/
	...
	{	
		...
		$userheaderlink_attrs = "href=\"".$this->tName."_list.php?orderby=Aemail\"";
		
		//for display name 
		$displayuserheaderlink_attrs = "href=\"".$this->tName."_list.php?orderby="."AName\"";
	...
	function save() 
	{
		if(postvalue("a")=="save")
		{
			...
			foreach(postvalue("username") as $user)
			{
				...
				$sql="delete from `ishaugmembers` where UserName=".db_prepare_string($user);
				...
				if(count($_POST["cb_".$useridx]))
				{
					...
					foreach(@$_POST["cb_".$useridx] as $g)
					{
						if($g<0)
						{
							if($user==$_SESSION["UserID"] && $g==-1)
								continue;
							$sql="insert into `ishaugmembers` (UserName,GroupID) values (".db_prepare_string(htmlspecialchars($user)).",".$g.")";
							db_exec($sql,$this->conn);
						}
						else
						{
					...
					if($glist!="")
					{
						$sql="insert into `ishaugmembers` (UserName,GroupID) select ".db_prepare_string(htmlspecialchars($user)).",GroupID from `ishauggroups` where GroupID in (".$glist.")";
						db_exec($sql,$this->conn);
					}
				}
			}
	...
	function fillGridData() 
	{
		...
		while($data && ($this->sortByGroup !== false || $this->recNo <= $this->pageSize || $this->pageSize==-1))
		{
			...
			foreach($this->groups as $idx => $g)
			{
				...
				if($_SESSION["UserID"] != $data["email"] || $g[0] != -1)
				{
			...
			//$row["begin"] = "<input type=hidden name=\"username[]\" value=\"".htmlspecialchars($data["email"])."\">";
			
			$row["username"] = htmlspecialchars($data["email"]);
			$row["displayusername"] = htmlspecialchars($data["Name"]); //htmlspecialchars($data["email"]);
			$row["user"] = $data["email"];
			$usercheckbox_attrs.=" name=\"user_".count($this->users)
				."\" id=\"".count($this->users)."\"";
			
			$this->users[] = htmlspecialchars($data["email"]);
	...
	{
		//	select members list		
		$trs = db_query("select UserName,GroupID from `ishaugmembers` order by UserName,GroupID",$this->conn);
	...
	function fillGroups() 
	{
		...
		$trs = db_query("select GroupID,Label from `ishauggroups` order by Label",$this->conn);
		while($tdata = db_fetch_numarray($trs))
		{
	...
	function setSessionVariables() 
	{
		...
		if (substr($_SESSION[$this->sessionPrefix."_orderby"],1) == "email" || substr($_SESSION[$this->sessionPrefix."_orderby"],1) =="Name"){
