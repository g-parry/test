	...
	function buildItegratedWhere($fieldsArr) 
	{
		...
		if(!($this->haveAgregateFields && $this->isUsedFieldsForSearch))
		{
			...
			{			
				...
				{
					$where = StrWhereExpression($this->_where[$this->sessionPrefix."simpleSrchFieldsComboOpt"], $simpleSrch
						, $this->_where[$this->sessionPrefix."simpleSrchTypeComboOpt"], "", $this->cipherer);
				...
				else 
				{
					for($i = 0; $i < count($this->searchFieldsArr); $i++) {
						...
						{
							$where = StrWhereExpression($this->searchFieldsArr[$i], $simpleSrch
								, $this->_where[$this->sessionPrefix."simpleSrchTypeComboOpt"], "", $this->cipherer);
							// add not 
							if($where && $this->_where[$this->sessionPrefix."simpleSrchTypeComboNot"])
							{
		...
		if (count($srchFields)){
			...
			{	
				...
				{
					$where = StrWhereAdv($srchF['fName'], $srchF['value1'], $srchF['opt'], $srchF['value2'], $srchF['eType'], false);							
					if($where)
					{
						// add not 
						if($srchF['not'])
