...
function ExportToXML($cipherer)
{
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["Name"] = GetData($row, "Name", "", PAGE_EXPORT);
			$values["email"] = GetData($row, "email", "", PAGE_EXPORT);
			$values["password"] = GetData($row, "password", "", PAGE_EXPORT);
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["ID"] = GetData($row, "ID", $format, PAGE_EXPORT);
				$format = "";
			$values["Name"] = GetData($row, "Name", $format, PAGE_EXPORT);
				$format = "";
			$values["email"] = GetData($row, "email", $format, PAGE_EXPORT);
				$format = "";
			$values["password"] = GetData($row, "password", $format, PAGE_EXPORT);
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
						...
						echo htmlspecialchars($values["password"]);
			echo '</td>';
			echo "</tr>";
		}
