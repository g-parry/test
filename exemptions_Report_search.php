...
$layout->blocks["top"][] = "fields";$page_layouts["exemptions_Report_search"] = $layout;
} else {
...
$layout->blocks["top"][] = "search";$page_layouts["exemptions_Report_search"] = $layout;
}
...
$mode = SEARCH_SIMPLE;
$templatefile = "exemptions_Report_search.htm";
...
$params['shortTableName'] = 'exemptions_Report';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("COUNT(Location)"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("COUNT(Location)")] = GetTableURL($pageObject->pSet->getLookupTable("COUNT(Location)"));
	
	$pageObject->fillFieldToolTips("COUNT(Location)");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("COUNT(Location)");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "COUNT(Location)";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("COUNT_Location__label","<label for=\"".GetInputElementId("COUNT(Location)", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("COUNT_Location__label", true);
	
	$xt->assign("COUNT_Location__fieldblock", true);
	$xt->assignbyref("COUNT_Location__editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("COUNT_Location__notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("COUNT_Location__editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_COUNT_Location_", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("COUNT(Location)");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"COUNT(Location)", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"COUNT(Location)", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
