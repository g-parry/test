...
$layout->blocks["top"][] = "details";$page_layouts["East_End_add"] = $layout;
} else {
...
if($inlineadd == ADD_INLINE)
	$templatefile = "East_End_inline_add.htm";
else
	$templatefile = "East_End_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: East_End_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'East_End_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'East_End_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: East_End_".$pageObject->getPageType().".php");
...
if($pageObject->isHTMLFormNeeded())
{
	...
	$pageObject->body["begin"] .= '<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="East_End_add.php">';
}
