...
$layout->blocks["top"][] = "fields";$page_layouts["isha_streets_search"] = $layout;
} else {
...
$layout->blocks["top"][] = "search";$page_layouts["isha_streets_search"] = $layout;
}
...
$mode = SEARCH_SIMPLE;
$templatefile = "isha_streets_search.htm";
...
$params['shortTableName'] = 'isha_streets';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("enfcompany"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("enfcompany")] = GetTableURL($pageObject->pSet->getLookupTable("enfcompany"));
	
	$pageObject->fillFieldToolTips("enfcompany");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("enfcompany");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "enfcompany";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("enfcompany_label","<label for=\"".GetInputElementId("enfcompany", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("enfcompany_label", true);
	
	$xt->assign("enfcompany_fieldblock", true);
	$xt->assignbyref("enfcompany_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("enfcompany_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("enfcompany_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_enfcompany", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("enfcompany");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"enfcompany", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"enfcompany", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
