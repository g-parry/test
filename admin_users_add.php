...
//	processing ID - start
	...
	if($inlineAddOption)
	{
		...
		if(!($value===false))
		{
						...
						if(0 && "ID"=="password" && $url_page=="admin_users_")
				$value=md5($value);
			$avalues["ID"]=$value;
		}
		}
...
//	Name
	...
	{	
		$value="";
			$value = ProcessLargeText($pageObject->pSet, GetData($data, "Name", "", PAGE_LIST),"field=Name".$keylink,"",MODE_LIST);
...
//	email
	...
	{	
		$value="";
			$value = ProcessLargeText($pageObject->pSet, GetData($data, "email", "", PAGE_LIST),"field=email".$keylink,"",MODE_LIST);
...
//	password
	...
	{	
		$value="";
			$value = ProcessLargeText($pageObject->pSet, GetData($data, "password", "", PAGE_LIST),"field=password".$keylink,"",MODE_LIST);
...
if($pageObject->isHTMLFormNeeded())
{
	...
	$pageObject->body["begin"] .= '<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="admin_users_add.php">';
}
