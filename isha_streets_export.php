...
$layout->blocks["top"][] = "export";$page_layouts["isha_streets_export"] = $layout;
} else {
$layout = new TLayout("export","FancyPink","MobilePink");
$layout->blocks["top"] = array();
$layout->containers["export"] = array();
...
$xt->display("isha_streets_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=isha_streets.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=isha_streets.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=isha_streets.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["ACTIVE"] = GetData($row, "ACTIVE", "", PAGE_EXPORT);
			$values["contract"] = GetData($row, "contract", "", PAGE_EXPORT);
			$values["enfcompany"] = GetData($row, "enfcompany", "", PAGE_EXPORT);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=isha_streets.csv");
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["Location"] = GetData($row, "Location", $format, PAGE_EXPORT);
				$format = "";
			$values["ACTIVE"] = GetData($row, "ACTIVE", $format, PAGE_EXPORT);
				$format = "";
			$values["contract"] = GetData($row, "contract", $format, PAGE_EXPORT);
				$format = "";
			$values["enfcompany"] = GetData($row, "enfcompany", $format, PAGE_EXPORT);
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["enfcompany"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Enfcompany").'</td>';	
	}
	else
	{
		echo "<td>"."ID"."</td>";
	...
	echo "</tr>";
		...
		$totalsFields[] = array('fName'=>"Location", 'totalsType'=>'', 'viewFormat'=>"");
			$totals["ACTIVE"] = array("value" => 0, "numRows" => 0);
		$totalsFields[] = array('fName'=>"ACTIVE", 'totalsType'=>'', 'viewFormat'=>"");
			$totals["contract"] = array("value" => 0, "numRows" => 0);
		$totalsFields[] = array('fName'=>"contract", 'totalsType'=>'', 'viewFormat'=>"");
			$totals["enfcompany"] = array("value" => 0, "numRows" => 0);
		$totalsFields[] = array('fName'=>"enfcompany", 'totalsType'=>'', 'viewFormat'=>"");
		...
		if ($eventRes)
		{
			...
			echo '</td>';
							echo '<td>';
									...
									$format = "";
									echo htmlspecialchars($values["ACTIVE"]);
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
			...
			echo '</td>';
					...
					echo '<td>';
									...
									$format = "";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["enfcompany"]);
					else
						echo htmlspecialchars($values["enfcompany"]);
			echo '</td>';
			echo "</tr>";
		}
