...
$layout->blocks["top"][] = "details";$page_layouts["exemptions_add"] = $layout;
} else {
...
if($inlineadd == ADD_INLINE)
	$templatefile = "exemptions_inline_add.htm";
else
	$templatefile = "exemptions_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: exemptions_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'exemptions_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'exemptions_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: exemptions_".$pageObject->getPageType().".php");
...
if($pageObject->isHTMLFormNeeded())
{
	...
	$pageObject->body["begin"] .= '<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="exemptions_add.php">';
}
