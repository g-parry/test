...
$layout->blocks["top"][] = "details";$page_layouts["isha_streets_edit"] = $layout;
} else {
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "isha_streets_inline_edit.htm" : "isha_streets_edit.htm";
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: isha_streets_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: isha_streets_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("enfcompany_label","<label for=\"".GetInputElementId("enfcompany", $id, PAGE_EDIT)."\">","</label>");
...
if($pageObject->isHTMLFormNeeded())
{
	...
	$pageObject->body["begin"] .= '<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="isha_streets_edit.php">';
}
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='isha_streets_view.php?".$viewlink."'\"");
