...
$layout->blocks["top"][] = "details";$page_layouts["Abbott_North_add"] = $layout;
} else {
...
if($inlineadd == ADD_INLINE)
	$templatefile = "Abbott_North_inline_add.htm";
else
	$templatefile = "Abbott_North_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: Abbott_North_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'Abbott_North_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'Abbott_North_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: Abbott_North_".$pageObject->getPageType().".php");
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "Abbott North";
		$options['firstTime'] = 1;
...
if($pageObject->isHTMLFormNeeded())
{
	...
	$pageObject->body["begin"] .= '<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="Abbott_North_add.php">';
}
