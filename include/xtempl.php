		...
		foreach($containersNames as $cname)
		{
			...
			foreach($container as $brick)
			{
				...
				if($this->hiddenBricks[$brick["name"]] 
					|| $brick["name"] == "wrapper" 
						&& (array_key_exists($brick["container"], $hidden_containers) || !array_key_exists($brick["container"], $displayed_containers))){
...
function xt_buildeditcontrol(&$params)
{
	...
	if(!$pageObj)
	{
		if(!$gSettings)
			...
			$pSet = $ptype != "" ? new ProjectSettings($gSettings->_table, $ptype) : $gSettings;
	}
	else 
	{
		$pSet = $pageObj->pSet;
	...
	if(@$params["format"]!="")
		...
		$format = is_null($pSet) ? "" : $pSet->getEditFormat($field);
	$append = "";
	...
	BuildEditControl($field, @$params["value"], $format, $mode, $ptype, $fieldNum, $id, $validate, $additionalCtrlParams, $pageObj);
}
