...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		foreach($arrFields as $value)
		{
			if(($pageObj->pSet->getEditFormat($value)==EDIT_FORMAT_LOOKUP_WIZARD || $pageObj->pSet->getEditFormat($value)==EDIT_FORMAT_RADIO) && ($pageObj->pSet->getpLookupType($value) == LT_LOOKUPTABLE || $pageObj->pSet->getpLookupType($value) == LT_QUERY))
			{
				$values[$value] = "";
				if(strlen($row[$value]))
					$values[$value] = DisplayLookupWizard($value, $row[$value], $row, "", MODE_EXPORT, PAGE_EXPORT);
			}
			elseif(IsBinaryType($pageObj->pSet->getFieldType($value)))
				$values[$value] = $row[$value];
			else
			{
				if($pageObj->pSet->getViewFormat($value)!=FORMAT_FILE_IMAGE && $pageObj->pSet->getViewFormat($value)!=FORMAT_FILE && $pageObj->pSet->getViewFormat($value)!=FORMAT_HYPERLINK && $pageObj->pSet->getViewFormat($value)!=FORMAT_EMAILHYPERLINK && $pageObj->pSet->getViewFormat($value)!=FORMAT_CHECKBOX)
					$format = $pageObj->pSet->getViewFormat($value);
				else
					$format = FORMAT_NONE;
				
				$values[$value] = GetData($row, $value, $format, PAGE_EXPORT);
			}
		}
		...
		if ($eventRes)
		{
			...
			foreach($arrFields as $value)
			{
				...
				elseif($pageObj->pSet->getViewFormat($value)==FORMAT_FILE_IMAGE)
