...
$layout->blocks["top"][] = "details";$page_layouts["isha_streets_add"] = $layout;
} else {
...
if($inlineadd == ADD_INLINE)
	$templatefile = "isha_streets_inline_add.htm";
else
	$templatefile = "isha_streets_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: isha_streets_add.php');
...
//	processing ACTIVE - start
	...
	if($inlineAddOption)
	{
		$value = postvalue("value_ACTIVE_".$id);
		$type=postvalue("type_ACTIVE_".$id);
		if (FieldSubmitted("ACTIVE_".$id))
		{
				$value=prepare_for_db("ACTIVE",$value,$type);
		}
		else
			$value=false;
		...
		if(!($value===false))
		{
						...
						if(0 && "ACTIVE"=="password" && $url_page=="admin_users_")
				$value=md5($value);
			$avalues["ACTIVE"]=$value;
		}
		}
//	processing ACTIVE - end
//	processing contract - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		...
		if(!($value===false))
		{
						...
						if(0 && "contract"=="password" && $url_page=="admin_users_")
				$value=md5($value);
			$avalues["contract"]=$value;
		}
		}
//	processing contract - end
//	processing enfcompany - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$value = postvalue("value_enfcompany_".$id);
		$type=postvalue("type_enfcompany_".$id);
		if (FieldSubmitted("enfcompany_".$id))
		{
				$value=prepare_for_db("enfcompany",$value,$type);
		}
		else
			$value=false;
		...
		if(!($value===false))
		{
						...
						if(0 && "enfcompany"=="password" && $url_page=="admin_users_")
				$value=md5($value);
			$avalues["enfcompany"]=$value;
		}
		}
//	processing enfcompany - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'isha_streets_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'isha_streets_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: isha_streets_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["contract"]=@$avalues["contract"];
	$defvalues["enfcompany"]=@$avalues["enfcompany"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("contract_label","<label for=\"".GetInputElementId("contract", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("enfcompany"))
		$xt->assign("enfcompany_fieldblock",true);
	else
		$xt->assign("enfcompany_tabfieldblock",true);
	$xt->assign("enfcompany_label",true);
	if(isEnableSection508())
		$xt->assign_section("enfcompany_label","<label for=\"".GetInputElementId("enfcompany", $id, PAGE_ADD)."\">","</label>");
...
//	ACTIVE
	...
	{	
		$value="";
			$value = ProcessLargeText($pageObject->pSet, GetData($data, "ACTIVE", "", PAGE_LIST),"field=ACTIVE".$keylink,"",MODE_LIST);
...
//	enfcompany
	...
	{	
		$value="";
			$value = ProcessLargeText($pageObject->pSet, GetData($data, "enfcompany", "", PAGE_LIST),"field=enfcompany".$keylink,"",MODE_LIST);
	$showValues["enfcompany"] = $value;
	$showFields[] = "enfcompany";
		$showRawValues["enfcompany"] = substr($data["enfcompany"],0,100);
	}	
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "isha_streets";
		$options['firstTime'] = 1;
...
if($pageObject->isHTMLFormNeeded())
{
	...
	$pageObject->body["begin"] .= '<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="isha_streets_add.php">';
}
