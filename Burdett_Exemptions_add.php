...
$layout->blocks["top"][] = "details";$page_layouts["Burdett_Exemptions_add"] = $layout;
} else {
...
if($inlineadd == ADD_INLINE)
	$templatefile = "Burdett_Exemptions_inline_add.htm";
else
	$templatefile = "Burdett_Exemptions_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: Burdett_Exemptions_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'Burdett_Exemptions_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'Burdett_Exemptions_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: Burdett_Exemptions_".$pageObject->getPageType().".php");
...
{
	$defvalues["Start Date"] = NOW();
	$defvalues["EndDate"] = Now();
}
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "Burdett Exemptions";
		$options['firstTime'] = 1;
...
if($pageObject->isHTMLFormNeeded())
{
	...
	$pageObject->body["begin"] .= '<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="Burdett_Exemptions_add.php">';
}
