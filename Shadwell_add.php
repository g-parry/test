...
$layout->blocks["top"][] = "details";$page_layouts["Shadwell_add"] = $layout;
} else {
...
if($inlineadd == ADD_INLINE)
	$templatefile = "Shadwell_inline_add.htm";
else
	$templatefile = "Shadwell_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: Shadwell_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'Shadwell_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'Shadwell_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: Shadwell_".$pageObject->getPageType().".php");
...
if($pageObject->isHTMLFormNeeded())
{
	...
	$pageObject->body["begin"] .= '<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="Shadwell_add.php">';
}
