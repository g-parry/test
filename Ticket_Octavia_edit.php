...
$layout->blocks["top"][] = "details";$page_layouts["Ticket_Octavia_edit"] = $layout;
} else {
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "Ticket_Octavia_inline_edit.htm" : "Ticket_Octavia_edit.htm";
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: Ticket_Octavia_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: Ticket_Octavia_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "Ticket Octavia";
		$options['firstTime'] = 1;
...
if($pageObject->isHTMLFormNeeded())
{
	...
	$pageObject->body["begin"] .= '<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="Ticket_Octavia_edit.php">';
}
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='Ticket_Octavia_view.php?".$viewlink."'\"");
