...
if(isMobile()) {
$layout = new TLayout("report_print","FancyPink","MobilePink");
...
$layout->blocks["top"][] = "grid";$page_layouts["exemptions_Report_rprint"] = $layout;
} else {
$layout = new TLayout("report_print","FancyPink","MobilePink");
$layout->blocks["top"] = array();
$layout->skins["pdf"] = "empty";
...
$params["templatefile"] = "exemptions_Report_rprint.htm";
$params["xt"] = &$xt;
...
else
{
	$rb = new Report(array($gQuery->HeadToSql(), $gQuery->FromToSql(), $strWhereClause, $gQuery->GroupByToSql(), $gQuery->HavingToSql())
		, $pageObject->pSet->GetTableData(".orderindexes"), $pageObject->searchClauseObj, $conn, $PageSize, 30, $params);
