...
$layout->blocks["top"][] = "details";$page_layouts["Ticket_Demo_Site_add"] = $layout;
} else {
...
if($inlineadd == ADD_INLINE)
	$templatefile = "Ticket_Demo_Site_inline_add.htm";
else
	$templatefile = "Ticket_Demo_Site_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: Ticket_Demo_Site_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'Ticket_Demo_Site_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'Ticket_Demo_Site_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: Ticket_Demo_Site_".$pageObject->getPageType().".php");
...
//	show readonly fields
	$readonlyfields["date"] = htmlspecialchars(GetData($defvalues, "date", "Short Date", PAGE_ADD));
$linkdata="";
...
if($pageObject->isHTMLFormNeeded())
{
	...
	$pageObject->body["begin"] .= '<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="Ticket_Demo_Site_add.php">';
}
