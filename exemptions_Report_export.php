...
$layout->blocks["top"][] = "export";$page_layouts["exemptions_Report_export"] = $layout;
} else {
$layout = new TLayout("export","FancyPink","MobilePink");
$layout->blocks["top"] = array();
$layout->containers["export"] = array();
...
$xt->display("exemptions_Report_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=exemptions_Report.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=exemptions_Report.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=exemptions_Report.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["COUNT(Location)"] = GetData($row, "COUNT(Location)", "", PAGE_EXPORT);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=exemptions_Report.csv");
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["Location"] = GetData($row, "Location", $format, PAGE_EXPORT);
				$format = "";
			$values["COUNT(Location)"] = GetData($row, "COUNT(Location)", $format, PAGE_EXPORT);
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["COUNT(Location)"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("COUNT(Location)").'</td>';	
	}
	else
	{
		echo "<td>"."Location"."</td>";
		echo "<td>"."COUNT(Location)"."</td>";
	}
	echo "</tr>";
			$totals = array();
		$totals["Location"] = array("value" => 0, "numRows" => 0);
		$totalsFields[] = array('fName'=>"Location", 'totalsType'=>'', 'viewFormat'=>"");
			$totals["COUNT(Location)"] = array("value" => 0, "numRows" => 0);
		$totalsFields[] = array('fName'=>"COUNT(Location)", 'totalsType'=>'', 'viewFormat'=>"");
		...
		if ($eventRes)
		{
			...
			echo '</td>';
							echo '<td>';
									...
									$format = "";
									echo htmlspecialchars($values["COUNT(Location)"]);
			echo '</td>';
			echo "</tr>";
		}
