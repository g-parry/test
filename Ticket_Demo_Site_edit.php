...
$layout->blocks["top"][] = "details";$page_layouts["Ticket_Demo_Site_edit"] = $layout;
} else {
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "Ticket_Demo_Site_inline_edit.htm" : "Ticket_Demo_Site_edit.htm";
...
//	processing date - begin
	...
	if($condition)
	{
		...
		if(FieldSubmitted("date_".$id))
		{
			...
			if($pageObject->pSet->getOwnerTable("date") != $strOriginalTableName)
		$value = false;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: Ticket_Demo_Site_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: Ticket_Demo_Site_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
//	show readonly fields
	$readonlyfields["date"] = htmlspecialchars(GetData($data, "date", "Short Date", PAGE_EDIT));
...
//	location - 
		$value = "";
				$value = DisplayLookupWizard("location", $data["location"], $data, $keylink, MODE_LIST, PAGE_LIST);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "Ticket Demo Site";
		$options['firstTime'] = 1;
...
if($pageObject->isHTMLFormNeeded())
{
	...
	$pageObject->body["begin"] .= '<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="Ticket_Demo_Site_edit.php">';
}
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='Ticket_Demo_Site_view.php?".$viewlink."'\"");
