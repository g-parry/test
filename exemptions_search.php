...
$layout->blocks["top"][] = "fields";$page_layouts["exemptions_search"] = $layout;
} else {
...
$layout->blocks["top"][] = "search";$page_layouts["exemptions_search"] = $layout;
}
...
$mode = SEARCH_SIMPLE;
$templatefile = "exemptions_search.htm";
...
$params['shortTableName'] = 'exemptions';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("contract"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("contract")] = GetTableURL($pageObject->pSet->getLookupTable("contract"));
	
	$pageObject->fillFieldToolTips("contract");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("contract");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "contract";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("contract_label","<label for=\"".GetInputElementId("contract", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("contract_label", true);
	
	$xt->assign("contract_fieldblock", true);
	$xt->assignbyref("contract_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("contract_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("contract_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_contract", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("contract");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"contract", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"contract", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
